...
* topdown stealth foundation modules
A modular stealth foundation is available under =stealth/= for top-down stealth gameplay.

- Full guide: =docs/stealth_foundation.md=
- Sample scene: =love test/stealth_demo=

This layer keeps collision/world setup in Breezefield while adding gameplay-oriented systems for:

- topdown movement
- guard proximity and line-of-sight checks
- trigger enter/stay/exit events
- debug rendering and data-driven tuning

** quick stealth snippets
*** load modules and create a stealth world
#+BEGIN_SRC lua
local bf = require("breezefield")
local stealth = require("stealth")
local cfg = require("stealth.config")

function love.load()
  local base_world = bf.newWorld(0, 0, true)
  world = stealth.World.new(base_world)
...
*** create a guard with perception
#+BEGIN_SRC lua
guard = world:createCollider({
  shape = "circle",
...
perception = stealth.Perception.new(world, guard, player, cfg.guard)
...
stealth.Triggers.newZone(world, {
  ...
  args = {500, 100, 120, 80},
  tag = "restricted",
  on_enter = function(zone, actor)
    print(actor.tag .. " entered restricted area")
  end,
})
#+END_SRC

*** update and debug draw
#+BEGIN_SRC lua
local move = stealth.Movement.new(player, cfg.player)
local debug = stealth.DebugDraw.new()
...
function love.keypressed(key)
  if key == action_key then debug:toggle() end
end
